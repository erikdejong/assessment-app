...
def format_metric_html(
    ...
    return (
        f'<div style="margin: 10px 0; padding: 15px; background-color: #f5f5f5; '
        f'border-radius: 8px; border-left: 5px solid {color};">'
        f'<div style="font-size: 14px; color: #666; margin-bottom: 5px;">{label}</div>'
        f'<div style="font-size: 28px; font-weight: bold; color: {color};">{value_str}</div>'
        f'</div>'
    )
...
def run_retrieval_evaluation(
    ...
    banner_div = (
        ...
        f'✓ Evaluation Complete: {count} tests</span></div>'
    )
    final_html = (
        '<div style="padding: 0;">'
        ...
        + format_metric_html(
            "Keyword Coverage", avg_coverage, "coverage", is_percentage=True
        )
        + banner_div
        + "</div>"
    )
...
async def run_answer_evaluation(
    ...
    final_html = (
        ...
        + format_metric_html(
            "Completeness", avg_completeness, "completeness", score_format=True
        )
